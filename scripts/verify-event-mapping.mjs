...
assert.ok(frontend.includes('Download Contacts (.vcf)'), 'WhatsApp group workflow must download event VCF contacts');
assert.ok(frontend.includes("Download Both Groups' Contacts"), 'WhatsApp group workflow must download combined VCF contacts');
...
assert.ok(frontend.includes('PST contacts are included only once'), 'Combined VCF download must explain PST de-duplication');
assert.ok(frontend.includes('Import the VCF'), 'WhatsApp group workflow must instruct contact import before WhatsApp group creation');
