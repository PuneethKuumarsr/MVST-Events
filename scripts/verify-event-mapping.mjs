...
const donorEligibility = readFileSync(new URL('../server/donorEligibility.js', import.meta.url), 'utf8');
...
assert.ok(frontend.includes("bhimaratha: 'BS'"), 'Bhimaratha receipt numbers must use BS prefix');
...
assert.ok(backend.includes('LEGACY_RECEIPT_PREFIXES'), 'Backend must still parse old BS26/SP26 receipt values');
...
assert.ok(frontend.includes('mangalyaDonorInvitationPackageReady'), 'Mangalya Donor queue must require identity, quantity and QR eligibility');
...
const legacyReceiptPrefixesForTest = {
  bhimaratha: 'BS26',
  shashtipoorthi: 'SP26',
};
const receiptNumberForTest = (receiptNo, eventType) => {
  const raw = String(receiptNo || '').trim();
  const prefixes = [receiptPrefixesForTest[eventType], legacyReceiptPrefixesForTest[eventType]].filter(Boolean).join('|');
...
assert.ok(backend.includes('crypto.randomBytes(15)'), 'QR tokens must be random and not encode participant data');
assert.ok(!backend.includes('crypto.createHmac'), 'QR tokens must not be deterministically derived from row or participant data');
...
assert.ok(frontend.includes('/api/auth/change-password'), 'Frontend must call backend change-password endpoint');
assert.ok(frontend.includes('function ChangePasswordSection'), 'Frontend must implement a simple Change Password screen');
assert.ok(frontend.includes('Current Password'), 'Change Password screen must ask for current password');
assert.ok(frontend.includes('New Password'), 'Change Password screen must ask for new password');
assert.ok(frontend.includes('Confirm New Password'), 'Change Password screen must ask to confirm the new password');
...
assert.ok(backend.includes("app.post('/api/auth/change-password', requireAuth"), 'Backend change-password endpoint must require authentication');
assert.ok(backend.includes('function changeOwnPassword'), 'Backend must change only the current user password');
assert.ok(backend.includes('verifyPin(currentPassword, currentUser.passwordHash)'), 'Change password must verify the current password first');
assert.ok(backend.includes('updateAuthUserRecord(currentUser.id, {') && backend.includes('pin: newPassword'), 'Change password must store the new password through the existing hash mechanism');
assert.ok(backend.includes('Session.deleteMany({ userId: currentUser.id })'), 'Change password must invalidate Mongo sessions for the current user');
assert.ok(backend.includes('clearSessionCookie(res)'), 'Change password must clear the current session cookie');
assert.ok(backend.includes("app.post('/api/users/:id/reset-password', requirePst"), 'PST admins must have a protected reset-password endpoint');
assert.ok(backend.includes('function resetUserPassword'), 'Backend must support PST owner password reset');
assert.ok(backend.includes('mustChangePassword: true'), 'New/reset users must be forced to change temporary passwords');
assert.ok(backend.includes('passwordAudit'), 'Password changes must maintain an audit log without passwords');
assert.ok(backend.includes('Use Reset Password to update another user password.'), 'Generic user edits must not reset passwords directly');
...
assert.ok(frontend.includes('Change Password'), 'Logged-in users must have Change Password in the menu');
assert.ok(frontend.includes('Password change required'), 'Frontend must show force-change-password status');
assert.ok(frontend.includes('/api/users/${encodeURIComponent(id)}/reset-password'), 'User Access must call reset-password endpoint');
assert.ok(frontend.includes('Reset Password'), 'User Access must expose Owner Reset Password action');
assert.ok(frontend.includes('Last Password Action'), 'User Access must show password audit metadata');
assert.ok(frontend.includes('Add Volunteer'), 'User Access must support adding volunteers');
assert.ok(frontend.includes('Reset Password'), 'User Access must support owner password resets');
...
assert.ok(frontend.includes("import trusteeInvitationCard from '../assets/invitations/mvst-samoohika-shanti-invitation-2026.jpg'"), 'Trustee bulk generator must bundle the approved invitation card');
assert.ok(eventInvitationTemplates.includes('*Special Greetings to Our Esteemed Members!*'), 'Trustee bulk generator must use the approved invitation message');
assert.ok(eventInvitationTemplates.includes('*Special Greetings to Our Esteemed Donors!*'), 'General and Mangalya donor invitation messages must address Esteemed Donors');
assert.ok((frontend.match(/DEFAULT_DONOR_INVITATION_MESSAGE/g) || []).length >= 3, 'Both donor invitation builders must use the Esteemed Donors template');
...
assert.ok(generalDonorIdentity.includes('function generalDonorFingerprint'), 'General Donor QR identity must use a stable donor fingerprint');
assert.ok(backend.includes("operation.donorFingerprint !== generalDonorFingerprint(donor)"), 'General Donor QR resolution must reject stale row-bound identities');
assert.ok(backend.includes("resetStaleQr ? { qrStatus: 'NOT_GENERATED', tokenRef: '', tokenHash: '' }"), 'General Donor campaign updates must invalidate QR tokens after an identity change');
assert.ok(backend.includes("recordType: 'DONOR'"), 'Distribution scanning must classify General Donor QR records');
assert.ok(backend.includes("app.get('/api/qr/donor/:token'"), 'Backend must expose authenticated General Donor QR verification');
assert.ok(frontend.includes("if (result.recordType === 'DONOR')"), 'QR operations UI must handle General Donor scan results');
assert.ok(frontend.includes('General Donor Verified'), 'QR operations UI must visibly confirm a General Donor scan');
...
assert.ok(backend.includes('const MANGALYA_RATE = 15000'), 'Backend must define the Mangalya donor rate server-side');
assert.ok(backend.includes('function normalizeMangalyaReceiptNumber'), 'Backend must normalize M001-style Mangalya receipt numbers');
...
assert.ok(backend.includes('tokenHash(tokenValue)'), 'Mangalya QR scan must resolve by token hash');
assert.ok(!backend.includes('MD:M001'), 'Mangalya QR must not use predictable MD:M001 payloads');
assert.ok(backend.includes("qrStatus === 'REVOKED'"), 'Revoked Mangalya QR tokens must be rejected');
assert.ok(backend.includes("recordType: 'MANGALYA'"), 'Distribution scan endpoint must return Mangalya record type');
assert.ok(backend.includes('MANGALYA_QR_GENERATED'), 'Backend must audit Mangalya QR generation');
assert.ok(backend.includes('MANGALYA_INVITATION_PREPARED'), 'Backend must audit Mangalya invitation preparation');
assert.ok(backend.includes('MANGALYA_ARRIVED'), 'Backend must audit Mangalya donor arrival');
assert.ok(backend.includes('MANGALYA_HONOURED'), 'Backend must audit Mangalya donor honouring');
assert.ok(backend.includes('MANGALYA_HONOUR_REVERSED'), 'Backend must audit Mangalya honour reversal');
assert.ok(backend.includes("honourStatus: { $ne: 'HONOURED' }"), 'Honour action must be idempotent and prevent duplicate honouring');
assert.ok(backend.includes("status: 'already-honoured'"), 'Duplicate honour scan must return already-honoured status');
assert.ok(backend.includes('maskForVolunteer'), 'Volunteer Mangalya scan response must mask mobile numbers');
assert.ok(frontend.includes('mangalyaDonorReceiptTemplate'), 'Frontend must import the approved Mangalya donor receipt asset');
assert.ok(frontend.includes('function generateMangalyaReceiptJpg'), 'Frontend must generate Mangalya donor receipt JPGs');
assert.ok(frontend.includes('Rate: ${formatCurrency(MANGALYA_RATE)}'), 'Mangalya receipt must display the rate');
assert.ok(frontend.includes('Qty: ${quantity}'), 'Mangalya receipt must display quantity');
assert.ok(frontend.includes('Total: ${formatCurrency(quantity * MANGALYA_RATE)}'), 'Mangalya receipt must display quantity-based total');
assert.ok(frontend.includes('function buildMangalyaInvitationMessage'), 'Frontend must build the Mangalya donor invitation message');
assert.ok(frontend.includes('4th Samoohika Shashtipoorthi Shanthi'), 'Mangalya invitation must contain 4th event text');
assert.ok(!frontend.includes('48th Samoohika'), 'Mangalya invitation must not contain 48th event text');
assert.ok(!frontend.includes('one Mangalya'), 'Mangalya invitation must not hardcode one Mangalya');
assert.ok(frontend.includes('Invitation Prepared, not delivered'), 'WhatsApp invitation must not be falsely marked delivered');
assert.ok(frontend.includes('Mangalya Donor Verified'), 'QR Operations must show a Mangalya donor verification card');
assert.ok(frontend.includes('Mark Arrived'), 'Mangalya donor scan card must support arrival marking');
assert.ok(frontend.includes('Mark Honoured'), 'Mangalya donor scan card must support honour marking');
assert.ok(frontend.includes('Already Honoured. Duplicate honouring is prevented.'), 'Mangalya donor scan card must prevent duplicate honouring');
assert.ok(mangalyaOperationModel.includes('mangalya_donor_operations'), 'Mangalya donor operation model must use its own collection');
assert.ok(mangalyaOperationModel.includes('tokenRef'), 'Mangalya operation must keep a private token reference for stable QR redraw');
assert.ok(mangalyaOperationModel.includes('tokenHash'), 'Mangalya operation must keep token hash for secure lookup');
assert.equal((mangalyaOperationModel.match(/tokenHash: \{ type: String/g) || []).length, 1, 'Mangalya operation schema must define tokenHash once');
assert.ok(mangalyaOperationModel.includes('receiptNumberNormalized'), 'Mangalya operation must index normalized receipt numbers');
assert.ok(mangalyaOperationModel.includes('eventYear: 1, donorSourceId: 1'), 'Mangalya operation must declare unique event/donor identity index');
assert.ok(mangalyaOperationModel.includes('eventYear: 1, receiptNumberNormalized: 1'), 'Mangalya operation must declare event receipt uniqueness index');
assert.ok(mangalyaOperationModel.includes('eventYear: 1, honourStatus: 1'), 'Mangalya operation must support honour status lookups');
assert.ok(mangalyaOperationModel.includes('eventYear: 1, arrivalStatus: 1'), 'Mangalya operation must support arrival status lookups');
assert.ok(mangalyaOperationModel.includes('arrivalStatus'), 'Mangalya operation must track arrival status');
assert.ok(mangalyaOperationModel.includes('honourStatus'), 'Mangalya operation must track honour status');
