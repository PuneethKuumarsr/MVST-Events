...
function boolValue(value) {
  ...
  return ['yes', 'true', '1', 'verified', 'received'].includes(textValue(value, '').trim().toLowerCase());
}
...
function mangalyaQuantity(donor) {
  const quantity = numberValue(donor?.quantity || donor?.confirmedQuantity || donor?.sponsored2026 || donor?.receivedQuantity || 0);
...
function isDirectBottuSponsor(sponsor) {
  const nature = textValue(sponsor?.contributionNature, '').toLowerCase();
  return nature.includes('material') || nature.includes('kind');
}

function mangalyaCashAmount(sponsor) {
  return isDirectBottuSponsor(sponsor) ? 0 : numberValue(sponsor?.confirmedAmount || sponsor?.amount || sponsor?.actualValue || 0);
}
...
function donorDonationType(donor) {
  ...
  const text = [donor.donorType, donor.contributionType, donor.category, donor.canonicalCategory]
    ...
    .join(' ')
    .toLowerCase();
  if (text.includes('mangalya') || numberValue(donor.confirmedQuantity || donor.sponsored2026 || donor.sponsored2025 || 0) > 0) {
...
function generalDonorInvitationPackageReady(donor) {
  return donorMobileIsValid(donor)
    && donorPaymentVerified(donor)
    && Boolean(donor.generalDonorSourceId || donor.id);
}
...
function makeMangalyaDonorWhatsAppUrl(donor, messageType = 'appeal') {
  ...
  const messageMap = {
    ...
    invitation: buildMangalyaDonorInvitationMessage,
...
function buildGeneralDonorMessage(donor, messageType = 'appeal') {
  const messageMap = {
    ...
    invitation: buildGeneralDonorInvitationMessage,
...
const TRUSTEE_INVITATION_CARD_FILENAME = 'MVST-Samoohika-Shanti-Invitation-02-Aug-2026.jpg';

function buildTrusteeMessage(contact, template = DEFAULT_TRUSTEE_MESSAGE) {
  return buildEventInvitationMessage(contact.name || 'Esteemed Trustee', template)
    .replaceAll('{{mobile}}', contact.mobileNumber ? `+${contact.mobileNumber}` : '')
    .replaceAll('{{role}}', contact.role || 'Trustee');
}

function buildGeneralDonorInvitationMessage(donor) {
  return buildEventInvitationMessage(sponsorDisplayName(donor), DEFAULT_DONOR_INVITATION_MESSAGE);
}

function buildMangalyaDonorInvitationMessage(donor) {
  return buildEventInvitationMessage(sponsorDisplayName(donor), DEFAULT_DONOR_INVITATION_MESSAGE);
}
...
function downloadEventInvitationCard() {
  ...
  link.download = TRUSTEE_INVITATION_CARD_FILENAME;
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
}
...
function downloadEventInvitationPackage(donor, donationType, qrDataUrl) {
  ...
  downloadBlob(dataUrlToBlob(qrDataUrl), donorQrPassFilename(donor, donationType));
}

function EventInvitationCardPackage({ includeQr = false }) {
  return (
    <div className="event-invitation-package">
      <a href={trusteeInvitationCard} target="_blank" rel="noreferrer" aria-label="Open the full invitation card">
        <img
          src={trusteeInvitationCard}
          alt="MVST Samoohika Shastipoorthi and Bheemaratha Shanti invitation card"
          loading="lazy"
          decoding="async"
        />
      </a>
      <div className="event-invitation-copy">
        <p>{includeQr ? 'Invitation + Personal QR Package' : 'Invitation Card Included'}</p>
    ...
    return (
      <section className="management-section section-error-fallback">
        ...
        <div className="receipt-modal-actions">
          {this.props.onRefresh ? <button type="button" onClick={this.resetAndRefresh}>Refresh Section</button> : null}
...
function safeSponsorForRender(sponsor = {}) {
  return {
    ...sponsor,
    id: textValue(sponsor.id, ''),
    rowNumber: numberValue(sponsor.rowNumber, 0),
    donorId: textValue(sponsor.donorId, ''),
    donorType: textValue(sponsor.donorType, ''),
    generalDonorSourceId: textValue(sponsor.generalDonorSourceId, ''),
    sponsorName: textValue(sponsor.sponsorName, ''),
    donorName: textValue(sponsor.donorName, ''),
    contactNo: textValue(sponsor.contactNo, ''),
    eventYear: textValue(sponsor.eventYear, ''),
    eventName: textValue(sponsor.eventName, ''),
    contributionType: textValue(sponsor.contributionType, ''),
    category: textValue(sponsor.category, ''),
    canonicalCategory: textValue(sponsor.canonicalCategory, ''),
    contributionNature: textValue(sponsor.contributionNature, ''),
    sponsored2025: numberValue(sponsor.sponsored2025 || sponsor.quantitySponsored || 0),
    quantitySponsored: numberValue(sponsor.quantitySponsored || sponsor.sponsored2025 || 0),
    sponsored2026: numberValue(sponsor.sponsored2026 || 0),
    confirmedQuantity: numberValue(sponsor.confirmedQuantity || sponsor.sponsored2026 || 0),
    receivedQuantity: numberValue(sponsor.receivedQuantity || 0),
    pendingQuantity: numberValue(sponsor.pendingQuantity || 0),
    confirmedAmount: numberValue(sponsor.confirmedAmount || sponsor.amount || 0),
    receivedAmount: numberValue(sponsor.receivedAmount || 0),
    amount: numberValue(sponsor.amount || sponsor.confirmedAmount || 0),
    actualValue: numberValue(sponsor.actualValue || 0),
    estimatedValue: numberValue(sponsor.estimatedValue || 0),
    balanceAmount: numberValue(sponsor.balanceAmount || 0),
    unit: textValue(sponsor.unit, ''),
    status: textValue(sponsor.status, 'Pending'),
    remarks: textValue(sponsor.remarks, ''),
    introducedBy: textValue(sponsor.introducedBy || sponsor.trusteeReference, ''),
    trusteeReference: textValue(sponsor.trusteeReference || sponsor.introducedBy, ''),
    paymentMode: textValue(sponsor.paymentMode, ''),
    bankOrCash: textValue(sponsor.bankOrCash, ''),
    paymentDate: textValue(sponsor.paymentDate, ''),
    collectedBy: textValue(sponsor.collectedBy, ''),
    receiptNumber: textValue(sponsor.receiptNumber, ''),
    qrStatus: textValue(sponsor.qrStatus, ''),
    arrivalStatus: textValue(sponsor.arrivalStatus, ''),
    honourStatus: textValue(sponsor.honourStatus, ''),
    honouredAt: textValue(sponsor.honouredAt, ''),
    appealSentDate: textValue(sponsor.appealSentDate, ''),
    sentDate: textValue(sponsor.sentDate, ''),
    confirmationSentDate: textValue(sponsor.confirmationSentDate, ''),
    paymentMessageSentDate: textValue(sponsor.paymentMessageSentDate, ''),
    postEventSentDate: textValue(sponsor.postEventSentDate, ''),
    whatsappSent: boolValue(sponsor.whatsappSent || sponsor.whatsAppSent),
    whatsAppSent: boolValue(sponsor.whatsAppSent || sponsor.whatsappSent),
    confirmationSent: boolValue(sponsor.confirmationSent),
    paymentMessageSent: boolValue(sponsor.paymentMessageSent),
    postEventSent: boolValue(sponsor.postEventSent),
    treasurerVerified: boolValue(sponsor.treasurerVerified),
    identityReady: sponsor.identityReady !== false,
    qrUrl: textValue(sponsor.qrUrl, ''),
  };
}

function safeRequirementForRender(row = {}) {
  return {
    ...row,
    id: textValue(row.id, ''),
    eventYear: textValue(row.eventYear, ''),
    category: textValue(row.category, ''),
    canonicalCategory: textValue(row.canonicalCategory, ''),
    unit: textValue(row.unit, ''),
    requiredQuantity: numberValue(row.requiredQuantity, 0),
    confirmedQuantity: numberValue(row.confirmedQuantity, 0),
    receivedQuantity: numberValue(row.receivedQuantity, 0),
    remainingQuantity: numberValue(row.remainingQuantity, 0),
    estimatedTotalCost: numberValue(row.estimatedTotalCost, 0),
...
function TrusteesSection({ trusteeState, user }) {
  ...
  function openTrustee(index, options = {}) {
        ...
        {queue.length ? (
          <div className="bulk-preview">
            <div className="bulk-preview-head">
              <div>
                <p>{campaignName}</p>
                <h3>{progress.total} Total · {progress.prepared} Prepared · {progress.sent} Sent · {progress.skipped} Skipped · {progress.failed} Failed · {progress.remaining} Remaining</h3>
              </div>
...
function PreviousDonorsCampaign({ donorState }) {
  ...
  const campaignName = 'General Donors Event Invitation – 2 August 2026';
  ...
  const invitationPackageReadyDonors = useMemo(
    () => visibleDonors.filter(generalDonorInvitationPackageReady),
    [visibleDonors],
  );
  const invitationQrBlockedDonors = useMemo(
    () => visibleDonors.filter((donor) => donorMobileIsValid(donor) && !generalDonorInvitationPackageReady(donor)),
    [visibleDonors],
  );
  ...
  useEffect(() => {
    ...
    previousDonors.forEach((donor) => {
      if (
        donor.previousDonorCampaignName === campaignName
        && donor.previousDonorCampaignStatus
        && donor.previousDonorCampaignStatus !== 'Pending'
      ) {
  ...
  function prepareQueue() {
    const latestStatus = readQueueStatus(campaignName);
    setStatusMap(latestStatus);
    setQueue(invitationPackageReadyDonors);
    setQueueStarted(false);
    ...
    setMessage(
      invitationPackageReadyDonors.length
        ...
        : 'No donors in this filter have both a valid WhatsApp number and an eligible personal QR.',
    );
  }
  ...
  async function recordPreviousDonorStatus(donor, statusValue, remarks = '') {
    ...
    const nextStatus = writeQueueStatus(campaignName, donor.id, statusEntry);
    setStatusMap(nextStatus);
    return nextStatus;
  }

  function advancePreviousDonorQueue(nextStatusMap, startIndex = queueIndex + 1) {
    const nextIndex = queue.findIndex((donor, index) => index >= startIndex && !['Sent', 'Prepared', 'Skipped'].includes(nextStatusMap[donor.id]?.status));
    if (nextIndex >= 0) {
      setQueueIndex(nextIndex);
      setQueueOpened(false);
    } else {
  ...
  async function prepareGeneralDonorInvitationPackage(donor) {
    const payload = donor.qrUrl
      ? { donor }
      : await prepareGeneralDonorQr(donor.id, {});
    const donorWithQr = {
    ...
    const qrDataUrl = await generateDonorQrPassJpg(donorWithQr, { donationType: 'Donor' });
    return {
      donorWithQr,
      qrDataUrl,
    };
  ...
  async function copyAllMessages() {
    const text = invitationPackageReadyDonors
      .map((donor, index) => [
        `${index + 1}. ${sponsorDisplayName(donor)}`,
        buildGeneralDonorInvitationMessage(donor),
      ].join('\n'))
      .join('\n\n------------------------------\n\n');
    try {
      await navigator.clipboard.writeText(text);
      setMessage(`Copied ${invitationPackageReadyDonors.length} personalized invitation messages.`);
    } catch (copyError) {
    ...
    <section className="management-section mangalya-donors-section previous-donors-section">
      ...
      <div className="controls sponsorship-controls previous-donor-controls">
        ...
        <button className="refresh-button compact" type="button" onClick={prepareQueue}>
          <MessageCircle size={16} /> Generate Invitation + QR Queue
        </button>
        <button className="refresh-button compact secondary-action" type="button" onClick={copyAllMessages} disabled={!invitationPackageReadyDonors.length}>
          Copy Invitation Messages
        </button>
      </div>
      ...
      <div className="bulk-whatsapp-panel donor-bulk-panel">
        <EventInvitationCardPackage includeQr />
        <div className="event-note invitation-package-readiness">
          <b>{invitationPackageReadyDonors.length} package-ready donors</b>
          <span>
            {invitationQrBlockedDonors.length} excluded until Treasurer Verified / Payment Received and personal QR eligibility are complete.
          </span>
        </div>
        {queue.length || queueStarted ? (
          <div className="bulk-preview">
            <div className="bulk-preview-head">
              <div>
                <p>General Donors Invitation + QR Queue</p>
                <h3>{previousDonorProgress.total} Total · {previousDonorProgress.prepared} Prepared · {previousDonorProgress.sent} Sent · {previousDonorProgress.skipped} Skipped · {previousDonorProgress.failed} Failed · {previousDonorProgress.remaining} Remaining</h3>
              </div>
            ...
            {queue.length ? (
              <>
                ...
                {currentQueueDonor ? (
                  <div className="donor-current-preview">
                    <div><p>Invitation Package Preview</p><strong>{sponsorDisplayName(currentQueueDonor)} - Invite Card + Personal QR</strong></div>
                    <textarea readOnly rows="16" value={buildGeneralDonorInvitationMessage(currentQueueDonor)} />
                  </div>
                ) : null}
                <div className="bulk-queue-controls">
            ...
            ) : <p className="bulk-empty">No eligible donors found. A valid mobile and Treasurer Verified / Payment Received status are required for the personal QR package.</p>}
          </div>
        ) : null}
      </div>
...
function MangalyaDonorsSection({ donorState, requirementState, requiredBottus = 0 }) {
  ...
  const [drilldownKey, setDrilldownKey] = useState('');
  const activeDonors = useMemo(() => donorRows.filter(isVisibleCurrentMangalyaSponsor).map(safeSponsorForRender), [donorRows]);
  const activeRequirements = useMemo(() => requirementRows.filter((row) => isActiveEventYear(row.eventYear)).map(safeRequirementForRender), [requirementRows]);
  ...
  const summary = useMemo(() => {
    ...
    const directBottus = directBottuSponsors.reduce((sum, sponsor) => sum + numberValue(sponsor.confirmedQuantity || sponsor.sponsored2026 || 0), 0);
    const confirmedAmount = cashSponsors.reduce((sum, sponsor) => sum + sponsorAmount(sponsor), 0);
    const receivedAmount = receivedSponsors.filter((sponsor) => !isDirectBottuSponsor(sponsor)).reduce((sum, sponsor) => sum + numberValue(sponsor.receivedAmount || sponsorAmount(sponsor) || 0), 0);
    const requirementQuantity = activeRequirements.reduce((sum, row) => sum + numberValue(row.requiredQuantity || 0), 0);
    return {
      ...
      topSponsors: [...confirmedSponsors]
        .sort((a, b) => numberValue(b.confirmedQuantity || b.sponsored2026 || 0) - numberValue(a.confirmedQuantity || a.sponsored2026 || 0))
        .slice(0, 4),
    };
  }, [activeDonors, activeRequirements, requiredBottus]);

  const requirementTotals = useMemo(() => activeRequirements.reduce((totals, row) => ({
    requiredQuantity: totals.requiredQuantity + numberValue(row.requiredQuantity || 0),
    confirmedQuantity: totals.confirmedQuantity + numberValue(row.confirmedQuantity || 0),
    receivedQuantity: totals.receivedQuantity + numberValue(row.receivedQuantity || 0),
    remainingQuantity: totals.remainingQuantity + numberValue(row.remainingQuantity || 0),
    estimatedTotalCost: totals.estimatedTotalCost + numberValue(row.estimatedTotalCost || 0),
    confirmedAmount: totals.confirmedAmount + numberValue(row.confirmedAmount || 0),
    receivedAmount: totals.receivedAmount + numberValue(row.receivedAmount || 0),
    remainingAmount: totals.remainingAmount + numberValue(row.remainingAmount || 0),
  }), {
    requiredQuantity: 0,
    confirmedQuantity: 0,
    receivedQuantity: 0,
  ...
  const financialTotals = useMemo(() => activeDonors.reduce((totals, sponsor) => {
    const received = numberValue(sponsor.receivedAmount || 0);
    const estimated = numberValue(sponsor.estimatedValue || 0);
    const directBottu = isDirectBottuSponsor(sponsor);
    const nature = textValue(sponsor.contributionNature, '').toLowerCase();
    const mode = textValue(sponsor.paymentMode || sponsor.bankOrCash, '').toLowerCase();
    if (directBottu) {
      totals.directBottuDonors += 1;
      totals.directBottuQuantity += numberValue(sponsor.confirmedQuantity || sponsor.sponsored2026 || 0);
  ...
  const drilldown = useMemo(() => {
    ...
    const drilldowns = {
      ...
      sponsorsPending: {
        ...
        rows: activeDonors.filter((sponsor) => textValue(sponsor.status, '').toLowerCase() === 'pending'),
  ...
  const visibleDonors = useMemo(() => {
    ...
    return activeDonors
      ...
      .filter((sponsor) => {
        ...
        if (quantityFilter === '3+') return numberValue(sponsor.sponsored2026 || 0) >= 3;
        return numberValue(sponsor.sponsored2026 || 0) === Number(quantityFilter);
      })
      .filter((sponsor) => {
        if (!search) return true;
  ...
  const invitationPackageReadySponsors = useMemo(
    () => visibleDonors.filter(mangalyaDonorInvitationPackageReady),
    [visibleDonors],
  );
  const invitationQrBlockedSponsors = useMemo(
    () => visibleDonors.filter((donor) => donorMobileIsValid(donor) && !mangalyaDonorInvitationPackageReady(donor)),
    [visibleDonors],
  );
  const invitationAlreadyPreparedSponsors = useMemo(
    () => visibleDonors.filter((donor) => mangalyaDonorInvitationPackageReady(donor) && donor.invitationPreparedAt),
    [visibleDonors],
  );
  ...
  function prepareBulkQueue() {
    ...
    setBulkMessage(
      invitationPackageReadySponsors.length
        ...
        : 'No Mangalya sponsors in this filter are ready for an invitation card + personal QR package.',
    );
    setBulkOpened(false);
  }
  ...
  async function prepareMangalyaInvitationPackage(donor) {
    const payload = donor.qrUrl
      ? { donor }
      : await prepareQr(donor.id, {});
    const donorWithQr = {
      ...donor,
      ...(payload.donor || {}),
      contactNo: donor.contactNo,
    };
    if (!donorWithQr.qrUrl) throw new Error('Mangalya donor QR could not be prepared.');
    const qrDataUrl = await generateDonorQrPassJpg(donorWithQr, { donationType: 'Mangalya Donor' });
    return {
      donorWithQr,
      qrDataUrl,
    };
  ...
  return (
    <section className="management-section mangalya-donors-section">
      ...
      <div className="bulk-whatsapp-panel donor-bulk-panel">
        <EventInvitationCardPackage includeQr />
        <div className="event-note invitation-package-readiness">
          <b>{invitationPackageReadySponsors.length} package-ready Mangalya donors</b>
          <span>
            {invitationQrBlockedSponsors.length} QR-ineligible · {invitationAlreadyPreparedSponsors.length} previously prepared package(s) included for verification.
          </span>
        </div>
        <div className="bulk-actions">
        ...
        {bulkQueue.length || bulkStarted ? (
          <div className="bulk-preview">
            <div className="bulk-preview-head">
              <div>
                <p>Mangalya Donors Invitation + QR Queue</p>
            ...
            {bulkQueue.length ? (
              <>
                ...
                {currentBulkDonor ? (
                  <div className="donor-current-preview">
                    <div><p>Invitation Package Preview</p><strong>{sponsorDisplayName(currentBulkDonor)} - Invite Card + Personal QR</strong></div>
                    <textarea readOnly rows="16" value={buildMangalyaDonorInvitationMessage(currentBulkDonor)} />
                  </div>
                ) : null}
                <div className="bulk-queue-controls">
...
function QRDistributionModule({ rows, writeEnabled, scanDistribution, user, isPst, initialScanToken = '' }) {
  ...
  async function handleTokenScan(token) {
    ...
    try {
      ...
      if (result.recordType === 'DONOR') {
        setScannedMangalyaDonor(null);
        setScannedGeneralDonor(result.donor);
        setScanPopup({
          type: 'success',
          title: 'General Donor Verified',
          participantName: result.donor?.donorName || 'General donor',
          eventLabel: 'General Donation',
          seatNo: '',
          operationLabel: 'Donor Verified',
          completedAt: result.completedAt || new Date().toISOString(),
          completedBy: operatorName,
        });
        setScanState({
          type: 'success',
          message: `General Donor Verified · ${result.donor?.donorName || 'Donor'} · ${formatCurrency(result.donor?.amount || 0)}`,
        });
        setManualToken('');
        return;
      }
    ...
    <section className="management-section qr-distribution-section">
      ...
      {scannedGeneralDonor ? (
        <div className="distribution-scanner-card mangalya-scan-result">
          <div className="distribution-scanner-head">
            <div>
              <span>✓ General Donor Verified</span>
              <strong>{scannedGeneralDonor.donorName || 'General donor'}</strong>
            </div>
            <StatusPill tone="success">QR Verified</StatusPill>
          </div>
          <div className="receipt-meta-grid">
            <p><span>Mobile</span>{scannedGeneralDonor.mobile || '-'}</p>
            <p><span>Contribution</span>{scannedGeneralDonor.contributionType || 'General Donation'}</p>
            <p><span>Amount</span>{formatCurrency(scannedGeneralDonor.amount || 0)}</p>
            <p><span>Payment Status</span>{scannedGeneralDonor.paymentStatus || '-'}</p>
            <p><span>Event Year</span>{scannedGeneralDonor.eventYear || ACTIVE_EVENT_YEAR}</p>
            <p><span>QR Status</span>{scannedGeneralDonor.qrStatus || 'ACTIVE'}</p>
          </div>
          <div className="receipt-queue-actions">
            <button type="button" onClick={() => setScannedGeneralDonor(null)}>Close</button>
          </div>
        </div>
      ) : null}
        ...
        <div className="participants-list">
          ...
          {activeView === 'mangalya-donors' && isPst && !mustChangePassword ? (
            <SectionErrorBoundary
              ...
              resetKey={`${donorState.lastRefreshedAt || ''}-${Array.isArray(donorState.donors) ? donorState.donors.length : 0}`}
